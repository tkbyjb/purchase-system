<template>
    <a-card style="width:100%;">
        ...
        <div class="flex align-items-center flex-wrap">
            <a-input
                v-model:value="searchParam.rolename"
                style="width:250px;"
                addon-before="角色名"
                addon-after
                allowClear
    ...
    <a-card style="width:100%;">
        <div class="flex align-items-center justify-content-between" style="margin-bottom:20px;">
            ...
            <span class="flex">
                ...
                >
                    <SvgIcon iconName="添加" :iconWidth="15" iconColor="white" />添加角色
                </el-button>
                <el-popconfirm
                ...
                <template #bodyCell="{ column, record }">
                    <template v-if="column.key === 'operation'">
                        <el-button
                            type="primary"
                            size="small"
                            @click="() => { visibleUpdate = true; role = record }"
                        >
                            <SvgIcon
                                iconName="edit"
                                :iconWidth="15"
    ...
    <a-modal
        ...
        style="width:800px;"
    >
        <RoleAdd @refresh="search" @click-close="() => { visibleAdd = false; }" />
    </a-modal>
</template>
...
import { RoleGetParam } from '@/type/role'
import RoleUpdate from '@/views/systemmanage/role/RoleUpdate.vue'
import RoleAdd from '@/views/systemmanage/role/RoleAdd.vue'
...
export default defineComponent({
    ...
    setup() {
        ...
        const searchParam: RoleGetParam = reactive({
            pageParam: new PageParam(1, 5),
            orderBys: [],
            rolename: '',
        });
        const item = ref<TableColumnsType>([
            { title: "角色名", dataIndex: "rolename", key: "rolename", resizable: true },
        ...
        function search(): void {
            proxy.$api.role.getRoles(searchParam)
                .then((response: any) => {
                    console.log(response.data.data)
                    data.lines = response.data.data.roles;
        ...
        function deletes(): void {
            ...
            proxy.$api.role.deleteRoles(selectRowKeys.value)
                .then((response: any) => {
                    //删除成功刷新表数据
