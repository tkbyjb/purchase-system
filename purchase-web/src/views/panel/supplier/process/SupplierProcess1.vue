<template>
  ...
  >
    <template #bodyCell="{ column, record }">
      <template v-if="['dealUnitPrice', 'dealTotalPrice'].includes(column.dataIndex)">
        <a-input-number v-if="record.isEdit"
                        ...
                        size="small"
                        style="width:150px;"
                        @blur="()=>{if(column.dataIndex=='dealUnitPrice'){record.dealTotalPrice=record.dealUnitPrice*record.count}}">
          <template #addonAfter>
            元
          </template>
        </a-input-number>
...
import {applyCanEditStateList, detailStateMap, purchaseStateToStepMap} from '@/util/state'
import NoteView from '@/components/NoteView.vue'
import type {TableColumnsType} from 'ant-design-vue';
import {Detail} from "@/type/detail";
import {ElMessage} from "element-plus";
import {Process2DetailParam} from "@/type/purchase";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    function clickToPrice(): void {
      //点击确认给出报价
      //必须所有明细给出报价
      let detailDeals = new Array<Process2DetailParam>()
      for (let d of details.data) {
        if(d.predictUnitPrice == null || d.dealTotalPrice == null) {
          ElMessage({
            type: 'warning',
            message: '必须给出每个明细价格'
          })
        ...
        detailDeals.push({
          ...
          dealTotalPrice: d.dealTotalPrice,
        })
      }
      proxy.$api.purchase.process2(purchaseIdC.value, detailDeals)
      .then((response: any)=>{
