<template>
  <el-descriptions :column="column" border size="large" title="申请详情">
    ...
    <el-descriptions-item :span="1">
      ...
      <el-button size="small" type="text"><a :href="reviewInfo.applyUnreviewVo.attachment" target="_blank">下载附件</a></el-button>
    </el-descriptions-item>
    <el-descriptions-item :span="column">
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    function toReview(): void {
      //点击确认审核按钮
      if(reviewParam.assignUserId == '' && reviewParam.result==1){
        ElMessage({
          type: 'warning',
          message: '请选择一个审核人'
        })
      ...
      proxy.$api.review.reviewAdd(levelC.value, reviewParam)
          .then((response: any) => {
    ...
    function getAssignUsers(): void {
      //获取可以审核的人列表
      proxy.$api.user.getAssignUsers(props.level+1)
          .then((response: any) => {
