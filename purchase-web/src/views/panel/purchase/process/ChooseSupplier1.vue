          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'operation'">
              ...
              <a-popconfirm
                  ...
                  title="确定选择此供应商?"
                  @cancel="()=>{}"
                  @confirm="chooseSupplier(record)"
              >
                <el-button size="small" style="margin-left:10px;" type="success"
                >选择
                </el-button>
              </a-popconfirm>
              <el-button size="small" style="margin-left:10px;" type="warning">发送消息</el-button>
            </template>
          </template>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    function chooseSupplier(record: any): void {
      //点击选择供应商,去更新父组件的数据
      context.emit('setSupplier', record)
    }
    let purchaseIdC = ref(props.purchaseId)
    function toXunjia(record: any): void {
      //点击去询价
      console.log(purchaseIdC.value, record.supplierId)
      proxy.$api.purchase.inquiryAdd(purchaseIdC.value, record.supplierId)
      // .then((response: any)=>{
      // //
      // })
    }
